...
import pe.edu.upeu.navegacionjpc.ui.presentation.component.CameraView
...
fun BarcodeScanningScreen(navController: NavController) {
    ...
    // Solicita el permiso si aún no se ha concedido
    LaunchedEffect(key1 = cameraPermissionState.hasPermission) {
    ...
    when {
        cameraPermissionState.hasPermission -> {
            // Si el permiso es otorgado, muestra el contenido
            ScanSurface(navController = navController)
        }
        cameraPermissionState.shouldShowRationale -> {
            // Si el usuario ha denegado el permiso anteriormente pero no lo ha bloqueado permanentemente
            Column {
                Toast.makeText(context, "Please grant camera permission to scan QR codes.", Toast.LENGTH_LONG).show()
            }
        }
        else -> {
            // Si el permiso es denegado de manera permanente
            Column {
                Toast.makeText(context, "Permission denied permanently. Please enable it from settings.", Toast.LENGTH_LONG).show()
            }
        }
    }
}*/
